...
func (s *Store) GetProxyConfig(ctx context.Context, nodeID string) (appdb.ProxyConfig, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT node_id, endpoint, token_encrypted, updated_at FROM proxy_config WHERE node_id = ?`, nodeID).
...
func (s *Store) UpsertProxyConfig(ctx context.Context, c appdb.ProxyConfig) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO proxy_config (node_id, endpoint, token_encrypted, updated_at)
		 VALUES (?, ?, ?, ?)
