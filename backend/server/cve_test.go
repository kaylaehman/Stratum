...
func TestCVEScansEmpty(t *testing.T) {
	...
	c := &http.Client{}
	resp, _ := c.Do(authReq(t, http.MethodGet, srv.URL+"/api/security/cve", token, nil))
...
func TestCVEScanUnknownContainer(t *testing.T) {
	srv, token := newNodeTestServer(t)
	c := &http.Client{}
	resp, _ := c.Do(authReq(t, http.MethodPost, srv.URL+"/api/containers/nope/cve-scan", token, nil))
	defer resp.Body.Close()
